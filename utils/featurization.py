...
def featurize_mol(mol, types=drugs_types):
    ...
    Returns:
        x:  node features, One-hot encoding
            Shape: [N, 44] for qm9
        z: atomic numbers of the nodes (the symbol one hot is included in x)
            Shape: [N]
        edge_index: [2, 2*E] tensor of node indices forming edges
    ...
    edge_attr = F.one_hot(edge_type, num_classes=len(bonds)+2).to(torch.float) # 4 bond types + 2 artificial types
